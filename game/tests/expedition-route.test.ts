...
test('grand chest has six valid deterministic high-tier rolls and independent claim bits',()=>{let epic=0,legendary=0,count=0;for(let seed=0;seed<100;seed++){const route=newExpeditionRoute(seed,20,1);route.cleared=9;const e=expeditionChoices(route)[0],items=expeditionRewardItems(e);assert.equal(items.length,6);assert.deepEqual(items,expeditionRewardItems(e));for(const i of items){assert.ok(validItem(i));assert.ok(['rare','epic','legendary','unique'].includes(i.tier));assert.equal(i.itemLevel,e.level+(i.tier==='unique'?0:3));epic+=Number(i.tier==='epic');legendary+=Number(i.tier==='legendary');count++;}assert.equal(dungeonChestMask({entrance:e} as never,2),127);}assert.ok(epic/count>.40&&epic/count<.58);assert.ok(legendary/count>.1&&legendary/count<.3);});
