...
import { quoteService, planService, respecCost, respecPoints } from '../src/commerce.ts';
...
test('respec refunds nodes and extra ranks exactly, without changing attributes or equipment',()=>{const p=setup(),before=structuredClone(p.character),count=respecPoints(before),q=quoteService(before,npc,p.level,{type:'respec'});assert.ok(q.ok);assert.equal(q.quote.price,respecCost(count));const result=planService(before,npc,p.level,q.quote);assert.ok(result.ok);assert.equal(result.character.skillPoints,19);assert.equal(result.character.gold,10000-respecCost(count));assert.deepEqual(result.character.allocatedNodes,['origin','wow-warrior-heroicStrike']);assert.deepEqual(result.character.skillSlots,Array(BAR_TOTAL).fill(null));assert.deepEqual(result.character.equipped,before.equipped);assert.deepEqual(result.character.attributes,before.attributes);assert.equal(planService(result.character,npc,p.level,q.quote).ok,false);assert.deepEqual(p.character,before);});
