...
test('ranks reward damage per mana and never lengthen cooldowns; utility ranks improve their role',()=>{
 const stats={manaCostMultiplier:1,cooldownMultiplier:1};for(const skill of Object.values(SKILL_DEFINITIONS).filter(d=>!d.classId&&!d.raceId)){
...
test('previous tree refunds nodes and paid ranks on a copy, preserving character and world progress',()=>{
 ...
 const original=JSON.stringify(save),upgraded=decodeCharacterSave(original);assert.ok(upgraded);assert.equal(JSON.stringify(save),original);const current=upgraded.checkpoint.character;assert.equal(current.treeVersion,SKILL_TREE_VERSION);assert.equal(current.treeRefunded,true);assert.equal(current.skillPoints,99);assert.deepEqual(current.allocatedNodes,['origin','wow-warrior-heroicStrike']);const slots:(SkillId|null)[]=Array.from({length:BAR_TOTAL},()=>null);slots[0]=WOW_CLASSES[current.classId].starterSkill;assert.deepEqual(current.skillSlots,slots);assert.deepEqual(current.equipped,s.equipped);assert.deepEqual(current.inventory,s.inventory);assert.equal(upgraded.checkpoint.level,100);assert.equal(upgraded.checkpoint.x,save.checkpoint.x);
...
test('every universal skill and its Techniques give useful, bounded upgrades through rank twenty',()=>{
 const {p,sheet}=setup();
 const immutable=JSON.stringify(SKILL_EXECUTION);
 for(const skill of Object.values(SKILL_DEFINITIONS).filter(d=>!d.classId&&!d.raceId)){
