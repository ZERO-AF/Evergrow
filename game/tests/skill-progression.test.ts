...
test('previous tree refunds nodes and paid ranks on a copy, preserving character and world progress',()=>{
 ...
 const original=JSON.stringify(save),upgraded=decodeCharacterSave(original);assert.ok(upgraded);assert.equal(JSON.stringify(save),original);const current=upgraded.checkpoint.character;assert.equal(current.treeVersion,SKILL_TREE_VERSION);assert.equal(current.treeRefunded,true);assert.equal(current.skillPoints,99);assert.deepEqual(current.allocatedNodes,['origin','wow-warrior-heroicStrike']);assert.deepEqual(current.skillSlots,Array.from({length:BAR_TOTAL},()=>null));assert.deepEqual(current.equipped,s.equipped);assert.deepEqual(current.inventory,s.inventory);assert.equal(upgraded.checkpoint.level,100);assert.equal(upgraded.checkpoint.x,save.checkpoint.x);
