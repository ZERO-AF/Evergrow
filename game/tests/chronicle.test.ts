...
test('lethal damage records actual life removed and one death; immunity cannot add damage',()=>{const sim=new Simulation(world,{spawn:false}),p=sim.player;p.hp=9;p.equipment.offHand=null;const context={player:p,world,random:()=>1,emit:(e:CombatEvent)=>trackChronicleEvent(p,[],e)};damageCombatant(999,0,1,'physical',context);damageCombatant(999,0,1,'physical',context);const v=chronicleValues(p.chronicle!.sources);assert.equal(v.damageTaken,9);assert.equal(v.deaths,1);});
