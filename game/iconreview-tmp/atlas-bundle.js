...
(() => {
  ...
  var WOW_RACIAL_SKILLS = Object.freeze(
    Object.fromEntries(RACIAL_SKILLS.map((s2) => [s2.raceId, s2]))
  );
  ...
  var SKILL_STATS = Object.freeze(Object.fromEntries(Object.values(SKILL_DEFINITIONS).map((s2) => [`skill:${s2.id}`, `${s2.name} ranks`])));
  var SKILL_AFFIXES = Object.freeze(Object.values(SKILL_DEFINITIONS).filter((s2) => s2.tier !== "aura").map((s2) => Object.freeze({ name: s2.name, stat: `skill:${s2.id}`, base: 1, growth: 0 })));
  ...
  // src/skill-tree-content.ts
  var SKILL_TERRITORIES = Object.freeze([
    { id: "bastion", name: "Bastion", domain: "Might", angle: -2.62, bend: -0.1, color: "#e5bd80", motto: "Endure \xB7 recover \xB7 retaliate" },
    { id: "forge", name: "Forge", domain: "Might", angle: 2.62, bend: 0.1, color: "#eb986f", motto: "Impact \xB7 reach \xB7 commitment" },
    { id: "hunt", name: "Hunt", domain: "Cunning", angle: -0.52, bend: -0.1, color: "#b9d990", motto: "Distance \xB7 projectiles \xB7 precision" },
    { id: "veil", name: "Veil", domain: "Cunning", angle: 0.52, bend: 0.1, color: "#79d5be", motto: "Movement \xB7 openings \xB7 tempo" },
    { id: "crucible", name: "Crucible", domain: "Arcana", angle: 1.57, bend: -0.07, color: "#c7a0ef", motto: "Flame \xB7 lightning \xB7 spellweave" },
    { id: "wellspring", name: "Wellspring", domain: "Arcana", angle: -1.57, bend: 0.07, color: "#8bd9ef", motto: "Frost \xB7 reserves \xB7 wards" }
  ]);
  var f = (name, small, reward, description) => ({ name, small, reward, description });
  var TERRITORY_SPECIALTIES = {
    bastion: [
      ...
      f("Second Breath", { lifeRegen: 0.15 }, { lifeRegen: 0.65, maxHp: 12 }, "Recover life steadily between hits."),
    ...
    forge: [
      ...
      f("Iron Harvest", { lifeOnHit: 0.25 }, { lifeOnHit: 1, armor: 24 }, "Direct-hit sustain supported by physical armor.")
    ],
    hunt: [
      ...
      f("Field Supplies", { manaRegen: 0.5 }, { manaOnKill: 2, potionPercent: 8 }, "Keep resources available during long hunts."),
      f("Arrowstorm", { areaPercent: 4 }, { areaPercent: 18, attackSpeedPercent: 7 }, "Wider ground coverage and faster bow recovery."),
      f("Heartseeker", { critDamage: 8, critChance: 0.75 }, { critDamage: 28, critChance: 2, damagePercent: 12 }, "Commit to powerful critical impacts."),
    ...
    crucible: [
      ...
      f("Conductive Vein", { manaRegen: 0.6 }, { manaRegen: 2.5, lightningResistance: 8 }, "Regenerate mana while insulating against lightning."),
    ...
    wellspring: [
      f("Clear Water", { manaRegen: 0.6 }, { manaRegen: 2.5, maxMana: 12 }, "Mana regeneration is displayed per five seconds."),
      ...
      f("Soul Stitch", { lifeRegen: 0.15 }, { lifeRegen: 0.6, arcaneResistance: 8 }, "Slow recovery with arcane protection."),
  ...
  var OUTER_SPECIALTIES = {
    bastion: [
      f("Stonefast", { armor: 8 }, { armor: 24, blockReduction: 4 }, "Physical armor with deeper shield absorption."),
      f("Ember Refuge", { fireResistance: 3 }, { fireResistance: 10, lifeRegen: 0.5 }, "Fire protection and steady life recovery."),
    ...
    veil: [
      ...
      f("Patient Blade", { critDamage: 8, critChance: 0.75 }, { critDamage: 24, critChance: 3, manaRegen: 1.5 }, "Prepare critical openings while mana returns."),
      f("Moonlit Recovery", { lifeRegen: 0.1 }, { lifeRegen: 0.5, cooldownPercent: 3 }, "Recover between openings and bring escapes back sooner.")
    ]
  };
  var BORDER_GARDENS = [
    { from: "bastion", to: "wellspring", specialties: [
      f("Mending Waters", { maxHp: 5 }, { lifeRegen: 0.5, manaRegen: 1.5 }, "Life and mana recovery for a patient defender."),
      ...
      f("Watchful Stillness", { lifeRegen: 0.1 }, { afterguardPercent: 16, manaRegen: 2 }, "Blocking reinforces armor while mana steadily returns.")
    ] },
    { from: "wellspring", to: "hunt", specialties: [
      f("Dewcatcher", { manaRegen: 0.4 }, { manaOnKill: 2, maxMana: 12 }, "Fund sustained ranged attacks with kills and a larger reserve."),
      ...
      f("Distant Echo", { critDamage: 8, critChance: 0.75 }, { areaPercent: 16, critChance: 2, critDamage: 16, manaOnKill: 2 }, "Critical investment with wider coverage and kill sustain.")
    ] },
    { from: "hunt", to: "veil", specialties: [
      ...
      f("Needlework", { critChance: 1.25, critDamage: 4 }, { critChance: 3, critDamage: 24, lifeOnHit: 0.75 }, "Critical precision with direct-hit recovery."),
    ...
    { from: "veil", to: "crucible", specialties: [
      ...
      f("Shadow Conduit", { intelligence: 2, spellDamagePercent: 3 }, { spellDamagePercent: 20, lifeOnHit: 0.75 }, "Stronger spells and recovery from direct contact.")
    ] },
    { from: "crucible", to: "forge", specialties: [
      ...
      f("Furnace Heart", { maxHp: 5 }, { manaOnKill: 2, lifeOnHit: 0.75 }, "Direct contact and kills replenish different resources."),
      f("Thunder Anvil", { critDamage: 8, critChance: 0.75 }, { strength: 4, critChance: 2, critDamage: 16, castSpeedPercent: 6 }, "Critical power with strength and faster casting.")
    ] },
    { from: "forge", to: "bastion", specialties: [
      ...
      f("Scar Tissue", { maxHp: 5 }, { lifeOnHit: 1, lifeRegen: 0.3 }, "Recover through direct hits and quiet moments."),
  ...
  function freezeContent(value) {
    Object.freeze(value);
    for (const child of Object.values(value)) if (child && typeof child === "object" && !Object.isFrozen(child)) freezeContent(child);
  }
  for (const territory of SKILL_TERRITORIES) freezeContent(territory);
  freezeContent(OUTER_SPECIALTIES);
  freezeContent(BORDER_GARDENS);
  freezeContent(TERRITORY_SPECIALTIES);
  freezeContent(SKILL_DOCTRINES);
  var s = (classId, domain, passives) => ({ classId, domain, skills: WOW_CLASS_SKILLS[classId], passives });
  var CLASS_SANCTUMS = [
    s("warrior", "Might", [
  ...
  // src/skill-tree-shapes.ts
  var cycle = (count) => Array.from({ length: count }, (_, i) => [i, (i + 1) % count]);
  var arc = (count, start, span, radius) => Array.from({ length: count }, (_, i) => {
    const angle = start + span * i / (count - 1);
    return [Math.cos(angle) * radius, Math.sin(angle) * radius];
  });
  var PASSIVE_CLUSTER_SHAPES = {
    ring: {
  ...
  function passiveClusterShape(small, reward) {
  ...
  function buildTree() {
    const nodes = [], edges = [], clusters2 = [];
    const link = (a, b) => {
      const x = byId2.get(a), y = byId2.get(b);
      if (x.neighbors.includes(b)) return;
      x.neighbors.push(b);
      y.neighbors.push(a);
    ...
    const linkGate = (a, b) => {
      ...
      x.neighbors.push(b);
      y.neighbors.push(a);
      edges.push({ from: a, to: b, classGate: true });
    };
    const byId2 = /* @__PURE__ */ new Map();
    const potential = /* @__PURE__ */ new Map();
    const add = (n, depth) => {
      if (byId2.has(n.id)) throw Error(n.id);
      const v = { ...n, bonuses: Object.freeze({ ...n.bonuses }), neighbors: [] };
      nodes.push(v);
      byId2.set(v.id, v);
      potential.set(v.id, depth);
      return v;
    };
    const distance = (a, b) => Math.hypot(a.x - b.x, a.y - b.y);
    const lens = (n) => n.kind === "major" ? 40 : n.kind === "notable" ? 24 : n.role === "travel" ? 12 : 18;
    const segmentDistance = (p, a, b) => {
      const dx = b.x - a.x, dy = b.y - a.y, d = dx * dx + dy * dy, t = d ? Math.max(0, Math.min(1, ((p.x - a.x) * dx + (p.y - a.y) * dy) / d)) : 0;
      return Math.hypot(p.x - a.x - t * dx, p.y - a.y - t * dy);
    };
    const cross = (a, b, c2) => (b.x - a.x) * (c2.y - a.y) - (b.y - a.y) * (c2.x - a.x);
    const intersects = (a, b, c2, d) => cross(a, b, c2) * cross(a, b, d) < -1e-5 && cross(c2, d, a) * cross(c2, d, b) < -1e-5;
    const clearLine = (a, b) => nodes.every((n) => n === a || n === b || segmentDistance(n, a, b) > lens(n) + 9) && edges.every((e) => e.from === a.id || e.to === a.id || e.from === b.id || e.to === b.id || !intersects(a, b, byId2.get(e.from), byId2.get(e.to)));
    const roadPoint = (t, depth) => ({ x: Math.cos(t.angle) * (240 + (depth - 1) * 80), y: Math.sin(t.angle) * (240 + (depth - 1) * 80) });
    const routeBonus = (t, i) => {
      ...
      return [{ [damage]: 5 }, { maxHp: 12, maxMana: 6 }, { [speed]: 2 }, { critChance: 0.75, critDamage: 4 }, { moveSpeedPercent: 1.5 }, { [damage]: 4, manaRegen: 0.5 }][i % 6];
    };
    const hubs = [];
    ...
    for (const t of SKILL_TERRITORIES) for (let i = 1; i <= 32; i++) {
      ...
      link(i === 1 ? "origin" : `road:${t.id}:${i - 1}`, n.id);
      hubs.push({ ...n, members: [n], territory: t, external: 0, road: true });
    }
    function pocket(parent2, t, side, make, depth, cluster) {
      let found;
      const aim = t.angle + side * Math.PI / 2;
      for (const reach of [135, 165, 195, 225, 255, 285]) {
        for (const offset of [0, 0.25, -0.25, 0.5, -0.5, 0.8, -0.8, 1.1, -1.1, 1.5, -1.5, 2, -2, Math.PI]) {
          const a = aim + offset, p = { x: parent2.x + Math.cos(a) * reach, y: parent2.y + Math.sin(a) * reach }, candidate = make(p, a);
          ...
          if (!candidate.every((n) => edges.every((e) => segmentDistance(n, byId2.get(e.from), byId2.get(e.to)) > (n.kind === "major" ? 55 : 35)))) continue;
          const pairs = cluster ? candidate.slice(1).map((n) => [candidate[0], n]) : candidate.map((n) => [parent2, n]);
          if (cluster) pairs.push([parent2, candidate[0]]);
          if (!pairs.every(([a2, b]) => nodes.every((n) => n.id === a2.id || n.id === b.id || segmentDistance(n, a2, b) > lens(n) + 9) && edges.every((e) => e.from === a2.id || e.to === a2.id || e.from === b.id || e.to === b.id || !intersects(a2, b, byId2.get(e.from), byId2.get(e.to))))) continue;
          found = candidate;
          break;
        }
        if (found) break;
      }
      if (!found) throw Error(`No compact pocket for ${parent2.id}`);
      const members = found.map((n, i) => add(n, depth + (cluster && i ? 1 : 0)));
      if (cluster) {
        link(parent2.id, members[0].id);
        for (const n of members.slice(1)) link(members[0].id, n.id);
        const x = members.reduce((s2, n) => s2 + n.x, 0) / members.length, y = members.reduce((s2, n) => s2 + n.y, 0) / members.length;
        clusters2.push({ id: cluster, name: members[0].name, domain: t.domain, territory: t.id, x, y, radius: Math.max(...members.map((n) => Math.hypot(n.x - x, n.y - y))) + 20 });
      } else for (const n of members) link(parent2.id, n.id);
    }
    for (const { t, skill, depth, side } of SKILL_TERRITORIES.flatMap((t2) => ACTIVE_ROUTES[t2.id].map(([skill2, depth2, side2]) => ({ t: t2, skill: skill2, depth: depth2, side: side2 }))).sort((a, b) => a.depth - b.depth)) {
      const def = SKILL_DEFINITIONS[skill], cluster = `development:${skill}`, variants = SKILL_SPECIALIZATIONS.filter((v) => v.skill === skill);
      pocket(byId2.get(`road:${t.id}:${depth}`), t, side, (p, a) => [
        ...
        ...variants.map((v, k) => ({ id: specializationNode(v.id), name: v.name, description: v.description, x: p.x + Math.cos(a + (k - 1) * 0.72) * 100, y: p.y + Math.sin(a + (k - 1) * 0.72) * 100, kind: "notable", domain: def.domain, territory: t.id, specialization: v.id, developmentSkill: skill, cluster, bonuses: {} }))
      ], depth + 1, cluster);
    }
    for (const t of SKILL_TERRITORIES) SKILL_DOCTRINES.filter((d) => d.territory === t.id).forEach((d, j) => {
      const depth = j ? 18 : 11;
      pocket(byId2.get(`road:${t.id}:${depth}`), t, j ? -1 : 1, (p, a) => d.choices.map((choice, k) => ({ id: `doctrine:${d.id}:${k}`, name: choice.name, description: `${d.name}: choose one of three. ${choice.description} Other choices in this family are mutually exclusive.`, x: p.x + Math.cos(a + Math.PI / 2) * (k - 1) * 70, y: p.y + Math.sin(a + Math.PI / 2) * (k - 1) * 70, kind: "notable", domain: t.domain, territory: t.id, doctrine: d.id, bonuses: choice.bonuses, role: "choice" })), depth + 1);
    });
    for (const [id, name, description, territory, depth] of [
      ...
      const t = SKILL_TERRITORIES.find((t2) => t2.id === territory);
      pocket(byId2.get(`road:${territory}:${depth}`), t, -1, (p) => [{ id, name, description, ...p, kind: "major", domain: t.domain, territory, keystone: true, bonuses: {} }], depth + 1);
    }
    const slots = [];
    for (let row = -9; row <= 9; row++) for (let col = -9; col <= 9; col++) {
      const x = (col + row % 2 * 0.5) * 350 + Math.sin(row * 7 + col * 3) * 16, y = row * 303 + Math.cos(row * 3 - col * 5) * 16, r = Math.hypot(x, y);
      if (r < 480 || r > 2700) continue;
      const p = { x, y };
      if (nodes.some((n) => distance(p, n) < (n.kind === "major" ? 220 : n.developmentSkill ? 178 : 155))) continue;
      if (edges.some((e) => segmentDistance(p, byId2.get(e.from), byId2.get(e.to)) < 132)) continue;
      slots.push(p);
    }
    const takeSlot = (target) => {
      ...
      if (best < 0) throw Error("Passive neighborhood packing exhausted");
      return slots.splice(best, 1)[0];
    };
    function neighborhood(id, f2, t, p, name = f2.name) {
      const shape = passiveClusterShape(f2.small, f2.reward), recipe = PASSIVE_CLUSTER_SHAPES[shape], rotation = Math.atan2(p.y, p.x) + Math.PI / 2;
      ...
      const members = recipe.points.map(([x, y], k) => {
        return add({ id: `${id}:${k}`, name: k === count - 1 ? name : `${name} \xB7 ${k + 1}`, description: k !== count - 1 && f2.reward.afterguardPercent ? "More armor. The endpoint enables Afterguard." : k === count - 1 && f2.reward.afterguardPercent ? "Enables Afterguard: more armor for 3s after blocking." : k !== count - 1 && f2.reward.spellweavePercent ? name === "Spellweave" ? "More mana. The endpoint enables Spellweave." : "More weapon and spell damage. The endpoint enables Spellweave." : f2.description, x: p.x + x * cos - y * sin, y: p.y + x * sin + y * cos, domain: t.domain, territory: t.id, kind: k === count - 1 ? "notable" : "minor", cluster: id, role: "cluster", bonuses: k === count - 1 ? f2.reward : f2.small }, depth);
      });
      for (const [a, b] of recipe.edges) link(members[a].id, members[b].id);
      clusters2.push({ id, name, domain: t.domain, territory: t.id, shape, ...p, radius: Math.max(...recipe.points.map(([x, y]) => Math.hypot(x, y))) + 20 });
      hubs.push({ id, members, territory: t, ...p, external: 0, road: false });
    }
    for (const t of SKILL_TERRITORIES) for (const [j, f2] of TERRITORY_SPECIALTIES[t.id].entries()) {
      ...
      neighborhood(`${t.id}:${j}`, f2, t, p);
    }
    for (const t of SKILL_TERRITORIES) for (const [j, f2] of (OUTER_SPECIALTIES[t.id] ?? []).entries()) neighborhood(`outer:${t.id}:${j}`, f2, t, takeSlot(roadPoint(t, 23 + j * 3)));
    for (const garden of BORDER_GARDENS) {
      const t = SKILL_TERRITORIES.find((t2) => t2.id === garden.from), other = SKILL_TERRITORIES.find((t2) => t2.id === garden.to);
      for (const [j, f2] of garden.specialties.entries()) {
        const a = roadPoint(t, 5 + j * 4), b = roadPoint(other, 5 + j * 4);
        neighborhood(`garden:${garden.from}:${garden.to}:${j}`, f2, j % 2 ? other : t, takeSlot({ x: (a.x + b.x) / 2, y: (a.y + b.y) / 2 }));
      }
    }
    const addedSubjects = [
      ...
      { name: "Critical Focus", small: { critChance: 0.8, critDamage: 6 }, reward: { critChance: 3, critDamage: 18 }, description: "Critical chance and damage improve both attacks and spell hits." },
      ...
      { name: "Flowing Power", small: { manaRegen: 0.8, spellDamagePercent: 3 }, reward: { manaRegen: 3, manaCostPercent: 4 }, description: "Sustain casting while developing spell damage." }
    ];
    ...
    for (const p of [...slots].sort((a, b) => Math.hypot(a.x, a.y) - Math.hypot(b.x, b.y) || a.x - b.x)) {
      ...
      const index = occurrences.get(t.id) ?? 0;
      occurrences.set(t.id, index + 1);
      const f2 = addedSubjects[(index + SKILL_TERRITORIES.indexOf(t)) % addedSubjects.length];
      const names = ["Inner", "Crossing", "Outer", "Far"];
      neighborhood(`neighborhood:${t.id}:${index}`, f2, t, p, `${t.name} ${names[Math.floor(index / 6) % names.length]} ${f2.name}`);
    }
    const parent = hubs.map((_, i) => i), find = (i) => parent[i] === i ? i : parent[i] = find(parent[i]);
    const firstRoad = hubs.findIndex((h) => h.road);
    hubs.forEach((h, i) => {
      if (h.road) parent[i] = firstRoad;
    });
    const candidates = [];
    for (let i = 0; i < hubs.length; i++) for (let j = 0; j < i; j++) {
      if (hubs[i].road && hubs[j].road) continue;
      const d = distance(hubs[i], hubs[j]);
      if (d < 680) candidates.push({ a: i, b: j, distance: d });
    }
    candidates.sort((a, b) => a.distance - b.distance || a.a - b.a || a.b - b.b);
    const linked = /* @__PURE__ */ new Set();
    const roadAttachments = /* @__PURE__ */ new Map();
    function connect(ai, bi, repair = false) {
      ...
      const pairs = a.members.flatMap((x) => b.members.map((y) => ({ x, y, d: distance(x, y) }))).sort((a2, b2) => a2.d - b2.d);
      for (const { x, y, d } of pairs) {
        if (x.neighbors.length >= 4 || y.neighbors.length >= 4 || !a.road && x.neighbors.some((id) => byId2.get(id).cluster !== x.cluster) || !b.road && y.neighbors.some((id) => byId2.get(id).cluster !== y.cluster) || !clearLine(x, y)) continue;
        ...
        for (const [i, p] of mids.entries()) {
          ...
          add({ id, name: `${t.name} crossing`, description: "A short connection between neighboring passive routes.", ...p, kind: "minor", role: "travel", domain: t.domain, territory: t.id, bonuses: routeBonus(t, i + ai + bi) }, Math.round(potential.get(x.id) + (potential.get(y.id) - potential.get(x.id)) * (i + 1) / steps));
          link(prev, id);
          prev = id;
        }
        link(prev, y.id);
        if (road.road) {
          const set = roadAttachments.get(cluster) ?? /* @__PURE__ */ new Set();
          set.add(road.territory.id);
          roadAttachments.set(cluster, set);
        }
        a.external++;
        b.external++;
    ...
    for (const c2 of candidates) if (find(c2.a) !== find(c2.b)) connect(c2.a, c2.b);
    for (const c2 of candidates) {
      const a = hubs[c2.a], b = hubs[c2.b];
      if (a.external >= (a.road ? 2 : 4) || b.external >= (b.road ? 2 : 4)) continue;
      connect(c2.a, c2.b);
    }
    for (const c2 of candidates) {
      const a = hubs[c2.a], b = hubs[c2.b];
      if ((!a.road && a.external < 2 || !b.road && b.external < 2) && a.external < 5 && b.external < 5) connect(c2.a, c2.b, true);
    }
    const isolated = hubs.filter((_, i) => find(i) !== find(firstRoad));
    if (isolated.length) throw Error(`Disconnected neighborhoods: ${isolated.map((h) => h.id).join(", ")}`);
    for (const t of SKILL_TERRITORIES) {
      const last = Math.max(...ACTIVE_ROUTES[t.id].map((v) => v[1]));
      for (let i = 32; i > last; i--) {
        const n = byId2.get(`road:${t.id}:${i}`);
        if (!n || n.neighbors.length !== 1) break;
        const neighbor = byId2.get(n.neighbors[0]);
        neighbor.neighbors.splice(neighbor.neighbors.indexOf(n.id), 1);
        edges.splice(edges.findIndex((e) => e.from === n.id || e.to === n.id), 1);
        nodes.splice(nodes.indexOf(n), 1);
        byId2.delete(n.id);
      }
    }
    for (const skill of AURA_IDS) {
      const aura = AURAS[skill], t = SKILL_TERRITORIES.find((t2) => t2.id === aura.territory), depth = Math.min(aura.points - 1, Math.max(...nodes.filter((n) => n.id.startsWith(`road:${t.id}:`)).map((n) => Number(n.id.split(":")[2]))));
      pocket(byId2.get(`road:${t.id}:${depth}`), t, skill === "elementalSpikes" ? 1 : -1, (p) => [{ id: `skill:${skill}`, name: aura.name, description: aura.description, ...p, kind: "major", domain: t.domain, territory: t.id, skill, cluster: `development:${skill}`, bonuses: {} }], depth + 1, `development:${skill}`);
    }
    {
      const sorted = [...SKILL_TERRITORIES].sort((a, b) => a.angle - b.angle);
      const cell = 100, grid = /* @__PURE__ */ new Map();
      const mark = (p) => {
      ...
      for (const e of edges) {
        const a = byId2.get(e.from), b = byId2.get(e.to), steps = Math.max(1, Math.ceil(distance(a, b) / 40));
        for (let i = 0; i <= steps; i++) mark({ x: a.x + (b.x - a.x) * i / steps, y: a.y + (b.y - a.y) * i / steps });
      }
      const gridClear = (c2, need) => {
        const reach = Math.ceil((need + cell * 1.5) / cell);
        for (let gx = Math.floor(c2.x / cell) - reach; gx <= Math.floor(c2.x / cell) + reach; gx++)
          for (let gy = Math.floor(c2.y / cell) - reach; gy <= Math.floor(c2.y / cell) + reach; gy++) {
            const list = grid.get(`${gx},${gy}`);
            if (!list) continue;
            for (const p of list) if (distance(c2, p) <= need) return false;
          }
        return true;
      };
      for (const sanctum of [...CLASS_SANCTUMS].sort((a, b) => b.skills.length - a.skills.length)) {
        ...
        const ringRadii = [];
        {
          ...
          for (const n of ringSizes) {
            r = Math.max(r + 44, n * 40 / (Math.PI * 2));
            ringRadii.push(r);
          }
        }
        const r2 = ringRadii[ringRadii.length - 1] + 33;
        const make = (center2) => {
          const members2 = sanctum.passives.map((p, i) => {
            const a = Math.atan2(-center2.y, -center2.x) + i * Math.PI * 2 / 3;
            return {
              id: `wow-${sanctum.classId}-passive-${i}`,
              name: p.name,
              description: p.description,
              x: center2.x + Math.cos(a) * 42,
              y: center2.y + Math.sin(a) * 42,
              kind: "notable",
              domain: sanctum.domain,
              classId: sanctum.classId,
              cluster: `sanctum:${sanctum.classId}`,
              role: "cluster",
              bonuses: p.small
            };
          });
          ...
          for (let ring = 0; ring < ringCount; ring++) {
            for (let i = 0; i < ringSizes[ring]; i++, idx++) {
              const skill = sanctum.skills[idx], a = Math.atan2(-center2.y, -center2.x) + i * Math.PI * 2 / ringSizes[ring];
              members2.push({
                id: `wow-${sanctum.classId}-${skill.id}`,
                name: skill.name,
                description: skill.description,
                x: center2.x + Math.cos(a) * ringRadii[ring],
                y: center2.y + Math.sin(a) * ringRadii[ring],
                kind: "notable",
                domain: skill.domain,
                classId: sanctum.classId,
                cluster: `sanctum:${sanctum.classId}`,
                ...
                skill: skill.id,
                bonuses: {},
                free: skill.id === WOW_CLASSES[sanctum.classId].starterSkill || void 0
              });
            }
          }
          return members2;
        ...
        for (let radius = 380; radius <= 3200 && !placed; radius += 15) {
          for (let deg = -180; deg < 180 && !placed; deg += 3) {
            const a = deg * Math.PI / 180, c2 = { x: Math.cos(a) * radius, y: Math.sin(a) * radius };
            if (!gridClear(c2, r2 + 20)) continue;
            if (!nodes.every((o) => distance(c2, o) > r2 + lens(o) + 20)) continue;
            if (!edges.every((e) => segmentDistance(c2, byId2.get(e.from), byId2.get(e.to)) > r2 + 20)) continue;
            const candidate = make(c2);
            if (candidate.every((n) => nodes.every((o) => distance(n, o) > 36 + lens(o)) && edges.every((e) => segmentDistance(n, byId2.get(e.from), byId2.get(e.to)) > 30))) {
              placed = candidate;
              center = c2;
            }
          }
        ...
        const passives = members.slice(0, 3);
        const rings = [];
        {
          let off = 3;
          for (const n of ringSizes) {
            rings.push(members.slice(off, off + n));
            off += n;
          }
        }
        ...
        for (const p of passives) for (const t of [...rings[0]].sort((a, b) => distance(p, a) - distance(p, b)).slice(0, 3)) link(p.id, t.id);
        ...
        const near = sorted.reduce((a, b) => Math.abs(Math.atan2(Math.sin(actual - a.angle), Math.cos(actual - a.angle))) < Math.abs(Math.atan2(Math.sin(actual - b.angle), Math.cos(actual - b.angle))) ? a : b);
        const far = sorted.find((t) => t !== near && Math.abs(Math.atan2(Math.sin(actual - t.angle), Math.cos(actual - t.angle))) < Math.PI / 2) ?? near;
        const anchors = [
          ...[depth, depth + 2, depth - 2, depth + 4, depth - 4, depth + 6, depth - 6, depth + 8].flatMap((d) => [byId2.get(`road:${near.id}:${d}`), byId2.get(`road:${far.id}:${d}`)]),
          byId2.get("origin"),
        ...
        clusters2.push({
          id: `sanctum:${sanctum.classId}`,
          name: `${WOW_CLASSES[sanctum.classId].name} Sanctum`,
          domain: sanctum.domain,
          classId: sanctum.classId,
          x: center.x,
          y: center.y,
    ...
    return Object.freeze({ nodes: Object.freeze(nodes), edges: Object.freeze(edges), clusters: Object.freeze(clusters2.map((c2) => Object.freeze(c2))), bounds });
  }
  var SKILL_TREE = buildTree();
  var SKILL_NODES = new Map(SKILL_TREE.nodes.map((n) => [n.id, n]));

  // iconreview-tmp/atlas-entry.ts
  var cv = document.getElementById("c");
  var c = cv.getContext("2d");
  c.fillStyle = "#0b1520";
  c.fillRect(0, 0, cv.width, cv.height);
  var clusters = SKILL_TREE.clusters.filter((cl) => cl.classId);
  var byId = new Map(SKILL_TREE.nodes.map((n) => [n.id, n]));
  var ox = 0;
  for (const cl of clusters) {
    ...
    c.strokeStyle = "#3a5a6a";
    c.lineWidth = 0.7;
    for (const e of SKILL_TREE.edges) {
      const a = byId.get(e.from), b = byId.get(e.to);
      if (a.cluster !== cl.id && b.cluster !== cl.id) continue;
      c.beginPath();
      c.moveTo(cx + (a.x - cl.x) * scale, cy + (a.y - cl.y) * scale);
      c.lineTo(cx + (b.x - cl.x) * scale, cy + (b.y - cl.y) * scale);
      c.stroke();
    }
    for (const n of members) {
      const x = cx + (n.x - cl.x) * scale, y = cy + (n.y - cl.y) * scale;
      c.beginPath();
      c.arc(x, y, n.skill ? 3.4 : 2.6, 0, Math.PI * 2);
      c.fillStyle = n.free ? "#ffe1a7" : WOW_CLASSES[cl.classId].color;
      c.fill();
      if (n.skill) {
        c.strokeStyle = "#0a1726";
        c.lineWidth = 0.8;
        c.stroke();
      }
    }
