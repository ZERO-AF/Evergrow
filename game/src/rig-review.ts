...
async function boot() {
  ...
  function draw() {
    ...
    const poseAt = (angle: number, moving: number, gaitPhase = Number(stride.value) * Math.PI * 2): CharacterPose => ({
      kind: 'player', angle, time: 1.25, moving, weapon: weapon.visual, raceId,
      grip: weapon.hands === 1 ? 'one-handed' : 'two-handed', appearance: raceId ? createRaceLook(raceId).appearance : DEFAULT_APPEARANCE, outfit,
