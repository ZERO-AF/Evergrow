...
import { dungeonTheme, DUNGEON_THEME_IDS, DUNGEON_THEMES, DUNGEON_EVENTS, type DungeonThemeId } from '../dungeon-content.ts';
...
let selectedTheme=(Object.hasOwn(DUNGEON_THEMES,params.get('theme')??'')?params.get('theme'):undefined) as DungeonThemeId|undefined;
...
function render(){
    ...
    if(view==='gallery'||view==='entrances'){
        ...
        if(view==='map'){
            ...
            const output=document.createElement('canvas');output.width=1440;output.height=1000;fx=new PostFX(output);renderer.reset();renderer.resize(720,500,1440,1000);renderer.cameraX=x;renderer.cameraY=y;
            let renderCount=0;
