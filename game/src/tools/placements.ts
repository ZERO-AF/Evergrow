...
function select(p:typeof places[number]|undefined){selected=p;drawMap();
  ...
  renderer.reset();renderer.resize(700,500,900,650);renderer.cameraX=p.x;renderer.cameraY=p.y-35;renderer.render(sim,world,0,{phase:'paused',reducedMotion:true});fx.render(renderer.canvas,0);
