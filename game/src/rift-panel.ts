...
import { bestRiftClear } from './leaderboard.ts';
...
export class RiftPanel {
  ...
  private render(){
    this.tooltip.hide();
    const ledger=this.state?.rifts??freshRiftLedger(),keys=this.player?.character.inventory.filter(i=>i?.kind==='riftKey'&&!i.locked)??[],top=bestRiftClear(ledger);
    this.element.innerHTML=`<header class="ui-window-header"><div><small>THE VEIL IS THIN</small><h2>Crimson Rift</h2></div><button class="ui-button" data-close aria-label="Close">×</button></header><div class="rift-body"><div class="rift-hero"><canvas aria-hidden="true" width="560" height="420"></canvas><div class="rift-records"><span><strong>${ledger.clears}</strong>Clears</span><span><strong>${ledger.highest||'—'}</strong>Highest level</span><span><strong>${top?`T${top.tier}`:'—'}</strong>Highest tier</span><span><strong data-best>—</strong>Best at this level</span></div><p>Ten minutes. A sea of monsters. One guardian.</p></div><div class="rift-setup"><div class="rift-level"><span>Rift level</span><strong data-level-label></strong></div><input data-level aria-label="Rift level offset" type="range" min="-10" max="10" step="1" value="${this.offset}"><div class="rift-range"><span>−10</span><span>Your level</span><span>+10</span></div><h3>Your rift keys <small>From your inventory · optional, single use</small></h3><div class="rift-keys"><button class="rift-key ${!this.selected?'selected':''}" data-key="" aria-pressed="${!this.selected}"><span>◇</span><small>No key</small></button>${keys.map(k=>`<button class="rift-key ${this.selected===k!.id?'selected':''}" data-key="${escapeUI(k!.id)}" data-tier="${k!.tier}" style="--key-color:${TIER_COLORS[k!.tier]}" aria-label="Tier ${k!.recipe.riftKeyTier} ${TIER_NAMES[k!.tier]} ${escapeUI(k!.name)}" aria-pressed="${this.selected===k!.id}">${itemPackIconSVG(k!,1,2)}<small>T${k!.recipe.riftKeyTier}</small></button>`).join('')}</div><div data-modifiers class="rift-modifiers"></div><div class="rift-reward"><span>Final chest</span><strong>Equipment · Gold · Guaranteed key</strong><small>Monster kills grant XP. All loot waits at the end.</small></div></div></div><footer class="ui-window-footer"><p role="status">Death or timeout ends the run. Pausing stops the clock.</p><button class="ui-button ui-primary" data-enter>Enter rift <span>→</span></button></footer>`;this.update();
  }
  private update(){if(!this.player)return;const level=Math.max(1,Math.min(1e6,this.player.level+this.offset)),key=this.player.character.inventory.find(i=>i?.id===this.selected);
    ...
    this.element.querySelector('[data-modifiers]')!.innerHTML=key?`<div class="hazard"><span>◆ Key tier ${key.recipe.riftKeyTier}</span><strong>×${Math.pow(RIFT_RULES.keyTierGrowth,key.recipe.riftKeyTier??1).toFixed(2)}</strong></div>`+modifiers.map(m=>`<div class="${m.beneficial?'boon':'hazard'}"><span>${m.beneficial?'✧':'◆'} ${m.label}</span><strong>+${m.value}${m.unit}</strong></div>`).join(''):'<p>An unkeyed rift. Unchanged monsters and rewards.</p>';
