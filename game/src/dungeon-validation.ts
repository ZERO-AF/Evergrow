...
export function validActors(v: unknown): v is StoredActor[] { return Array.isArray(v) && v.every(a => object(a) && typeof a.kind === 'string' && Object.hasOwn(ENEMY_DEFINITIONS, a.kind) && ['normal', 'veteran', 'elite', 'rare'].includes(a.rank as string) && integer(a.level, 1, 1e6) && typeof a.biome === 'string' && Object.hasOwn(BIOMES, a.biome) && integer(a.seed, 0, 4294967295) && point(a) && number(a.homeX, -4e7, 4e7) && number(a.homeY, -4e7, 4e7) && (a.rift===undefined||validRiftTag(a.rift)) && (a.faction===undefined||isFactionTag(a.faction)) && number(a.hp, 0, riftEnemyStats(scaledEnemyStats(a.kind as StoredActor['kind'], a.level as number, a.rank as StoredActor['rank']),a.rift as StoredActor['rift']).maxHp) && (a.campId === undefined || text(a.campId, 180) && text(a.memberId, 180)) && (a.bossPhases === undefined || integer(a.bossPhases, 0, 3))); }
...
export function validExpeditions(v: unknown): v is Expeditions {
    ...
    for (const run of v.runs) {
        ...
        if ((e.scaling !== undefined && (!validEncounterScale(e.scaling) || e.level !== e.scaling.base)) || !text(e.id, 180) || !(e.id.startsWith('dungeon:') || e.id.startsWith('atlas:')) || ids.has(e.id) || !text(e.name, 80) || !point(e) || !integer(e.seed, 0, 4294967295) || !integer(e.level, 1, 1e6) || typeof e.biome !== 'string' || !Object.hasOwn(BIOMES, e.biome) || !object(run.states) || !validContents(run.contents) || !point(run))
            return false;
        if(e.rift!==undefined){
