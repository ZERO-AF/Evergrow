import { validRiftTag, validRiftLedger, RIFT_RULES } from './rift-content.ts';
...
import { isFactionTag } from './factions.ts';
const point = (v: Record<string, unknown>) => number(v.x, -4e7, 4e7) && number(v.y, -4e7, 4e7);
export function validActors(v: unknown, heroic = false): v is StoredActor[] { return Array.isArray(v) && v.every(a => object(a) && typeof a.kind === 'string' && Object.hasOwn(ENEMY_DEFINITIONS, a.kind) && ['normal', 'veteran', 'elite', 'rare'].includes(a.rank as string) && integer(a.level, 1, 1e6) && typeof a.biome === 'string' && Object.hasOwn(BIOMES, a.biome) && integer(a.seed, 0, 4294967295) && point(a) && number(a.homeX, -4e7, 4e7) && number(a.homeY, -4e7, 4e7) && (a.rift===undefined||validRiftTag(a.rift)) && (a.faction===undefined||isFactionTag(a.faction)) && number(a.hp, 0, applyEnemyModifiers(scaledEnemyStats(a.kind as StoredActor['kind'], a.level as number, a.rank as StoredActor['rank']),{kind:a.kind as StoredActor['kind'],rank:a.rank as StoredActor['rank'],rift:a.rift as StoredActor['rift'],heroic}).maxHp) && (a.campId === undefined || text(a.campId, 180) && text(a.memberId, 180)) && (a.bossPhases === undefined || integer(a.bossPhases, 0, 31))); }
...
export function validContents(v: unknown, heroic = false): v is LocationContents { return object(v) && (v.encounterScales === undefined || validEncounterScales(v.encounterScales)) && (v.campWounds === undefined || validCampWounds(v.campWounds)) && validActors(v.actors, heroic) && validPickups(v.pickups) && Array.isArray(v.groundItems) && v.groundItems.length <= LOOT_RULES.maxGroundItems && v.groundItems.every(i => object(i) && integer(i.id, 1) && point(i) && validItem(i.item) && validTreasureFlight(i.flight)) && Array.isArray(v.groundGold) && v.groundGold.length <= 128 && v.groundGold.every(i => object(i) && integer(i.id, 1) && point(i) && integer(i.amount, 1) && validTreasureFlight(i.flight) && number(i.age, 0, 10)) && Array.isArray(v.clearedCamps) && v.clearedCamps.every(id => text(id, 180)) && object(v.defeatedCampMembers) && Object.entries(v.defeatedCampMembers).every(([k, a]) => text(k, 180) && Array.isArray(a) && a.length <= 32 && a.every(id => text(id, 180))); }
export function validExpeditions(v: unknown): v is Expeditions {
    if (!object(v) || !(v.location === null || text(v.location, 180)) || !Array.isArray(v.runs) || !(v.surface === null || validContents(v.surface)) || !number(v.surfaceX, -4e7, 4e7) || !number(v.surfaceY, -4e7, 4e7))
        return false;
    if (v.cleared !== undefined && (!Array.isArray(v.cleared) || !v.cleared.every(id => text(id, 180) && (id.startsWith('dungeon:') || id.startsWith('atlas:'))) || new Set(v.cleared).size !== v.cleared.length)) return false;
    ...
    for (const run of v.runs) {
        ...
        if ((e.scaling !== undefined && (!validEncounterScale(e.scaling) || e.level !== e.scaling.base)) || !text(e.id, 180) || !(e.id.startsWith('dungeon:') || e.id.startsWith('atlas:')) || ids.has(e.id) || !text(e.name, 80) || !point(e) || !integer(e.seed, 0, 4294967295) || !integer(e.level, 1, 1e6) || typeof e.biome !== 'string' || !Object.hasOwn(BIOMES, e.biome) || !object(run.states) || !validContents(run.contents, (e.scaling as { heroic?: boolean } | undefined)?.heroic === true) || !point(run))
            return false;
        if(e.rift!==undefined){
        ...
        if (Object.keys(run.states).length !== floor.members.length || !floor.members.every(m => { const s = (run.states as Record<string, unknown>)[m.id]; const ent = e as unknown as DungeonEntrance, rank = dungeonMemberRank(ent, m); return object(s) && number(s.hp, 0, applyEnemyModifiers(scaledEnemyStats(m.kind, dungeonMemberLevel(ent, m), rank), { kind: m.kind, rank, rift: ent.rift, heroic: ent.scaling?.heroic }).maxHp) && point(s) && typeof s.admitted === 'boolean' && !dungeonBlocked(floor, s.x as number, s.y as number, 0) && (s.bossPhases === undefined || integer(s.bossPhases, 0, 31)); }))
            return false;
        if(e.rift!==undefined){
