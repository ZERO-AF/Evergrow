...
export function validActors(v: unknown): v is StoredActor[] { return Array.isArray(v) && v.every(a => object(a) && typeof a.kind === 'string' && Object.hasOwn(ENEMY_DEFINITIONS, a.kind) && ['normal', 'veteran', 'elite'].includes(a.rank as string) && integer(a.level, 1, 1e6) && typeof a.biome === 'string' && Object.hasOwn(BIOMES, a.biome) && integer(a.seed, 0, 4294967295) && point(a) && number(a.homeX, -4e7, 4e7) && number(a.homeY, -4e7, 4e7) && (a.rift===undefined||validRiftTag(a.rift)) && (a.faction===undefined||isFactionTag(a.faction)) && number(a.hp, 0, riftEnemyStats(scaledEnemyStats(a.kind as StoredActor['kind'], a.level as number, a.rank as StoredActor['rank']),a.rift as StoredActor['rift']).maxHp) && (a.campId === undefined || text(a.campId, 180) && text(a.memberId, 180)) && (a.bossPhases === undefined || integer(a.bossPhases, 0, 3))); }
