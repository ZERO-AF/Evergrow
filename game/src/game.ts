...
export class Game {
  ...
  private frame = (now: number) => {
    ...
    if (this.phase !== 'ready' && !dungeonRun && this.renderer.navigationVisible) {
      const bounds = minimapWorldBounds(mapPlayer, this.renderer.width, this.renderer.height);
      if (GAME_FEATURES.minimapTracking) {
        ...
        drawTrackingBlips(ui, minimapView(mapPlayer, this.renderer.width, this.renderer.height), blips,
          (x, y) => this.exploration.isRevealed(x, y));
        const questView = minimapView(mapPlayer, this.renderer.width, this.renderer.height);
        for (const marker of questMapMarkers(this.world, this.sim.player)) drawQuestMapMarker(ui, questView, marker);
