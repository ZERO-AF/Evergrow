...
export function hairShapes(style:Hair, p:AppearancePalette, facing:number):{rear:GearShape[];front:GearShape[]} {
  ...
  if(length) {
    rear.push(fill([[-4.2,-3],[4.2,-3],[4.8,2],[4.9,length-2],[3.5,length-.4],[1.9,length],[0,length-.8],[-2.1,length-.2],[-4.4,length-1.4],[-4.8,2]].map(([x,y]):Point=>[mx(x),y]),p.shadow));
    for(const x of [-3.9,3.9]) rear.push(fill([[mx(x-.8),-1],[mx(x+.6),-1],[mx(x+1),length-1.6],[mx(x-.8),length-1]],p.base),line([[mx(x),-.2],[mx(x+.25),length-2]],p.light,.45));
  }
  if(style==='ponytail'||style==='lowbun') {
    // Canonical right-facing anchor; mx mirrors the finished points so west is exact.
    const x=style==='ponytail'?-4:-3.8;
    const bunY=Math.round((3.7-Math.max(0,Math.sin(facing))*2.5)*1e9)/1e9;
    rear.push(fill(style==='lowbun'?cluster(x,bunY,2.8).map(([px,py])=>[mx(px),py]):[[x-1.7,-1],[x+1.5,-1],[x+2.8,4],[x+1.8,9.5],[x-.6,11],[x-1.1,5],[x-2,2]].map(([px,py])=>[mx(px),py]),p.shadow));
    if(style==='ponytail')rear.push(line([[mx(x-.6),1],[mx(x+.4),4],[mx(x+.3),8.5]],p.light,.65));
  }
  ...
  const cap=(simple[style]??[[-4.5,.7],[-4.6,-2.6],[-3,-5],[.5,-5.5],[3.7,-3.4],[4.5,-.6],[4,3.8],[2.9,2.4],[2.3,-1.9],[.3,-3.1],[-.8,-1.6],[-2.9,-.4],[-3.6,4]]).map(([x,y]):Point=>[mx(x),y]);
  if(style==='curls'||style==='afro') {
    const r=style==='afro'?6.3:5.1, y=style==='afro'?-2.2:-2;
    front.push(fill(cluster(0,y,r,style==='afro'?12:8).map(([px,py])=>[mx(px),py]),p.shadow));
    ...
    const crown:Point[]=Array.from({length:25},(_,i)=>{
      ...
      return [mx(Math.cos(a)*radius),y+Math.sin(a)*radius];
    });
    front.push(fill([...crown,[mx(r),1.2],[mx(3.4),2.3],[mx(2.8),-.9],[mx(1.2),-1.9],[mx(0),-2.2],[mx(-1.3),-1.3],[mx(-2.7),-1.7],[mx(-3.2),2],[mx(-r),1.1]],p.base));
    // Clear the central cluster fill from the face by moving its full silhouette behind the head.
    rear.push(front.shift()!);
    for(const [x,yy] of [[-4,-3.7],[-2,-5.3],[.5,-5.7],[3,-4.7],[4.4,-2.2],[-4.7,-.4]]) front.push(line([[mx(x-.5),yy],[mx(x),yy-.5],[mx(x+.7),yy-.2]],p.light,.55));
  } else if(style==='waves') {
    front.push(fill([[-5,6],[-4.4,3],[-5,.5],[-4.7,-2.6],[-3.1,-5],[.5,-5.6],[3.8,-3.5],[4.7,-.5],[4.2,2.2],[5.2,5],[4.4,9.6],[3.2,10],[3.5,5.5],[2.8,2],[2.4,-1.7],[.5,-3],[-1.4,-1.6],[-3,-.4],[-3.1,3],[-3.8,6],[-3.1,9.7],[-4.8,10.5]].map(([x,y]):Point=>[mx(x),y]),p.base));
    for(const s of [-1,1]) front.push(line([[mx(s*2.4),-3],[mx(s*3.8),-.3],[mx(s*3.4),2.8],[mx(s*4.2),5.5],[mx(s*3.8),8.9]],p.light,.55));
  } else if(style==='locs') {
    ...
    front.push(fill([[-4.6,1.4],[-4.6,-2.2],[-3.2,-4.8],[-.7,-5.5],[2,-4.9],[4,-3],[4.5,1.5],[3.1,1],[2.4,-1.7],[.5,-2.6],[-1.4,-1.5],[-3,.5]].map(([x,y]):Point=>[mx(x),y]),p.base));
    for(const side of [-1,1]) for(let i=0;i<2;i++) {
      const x=side*(3.4+i*.95),bottom=8.6+i*.9;
      front.push(fill([[mx(x-.6),-2.5],[mx(x+.45),-2.9],[mx(x+.65),1.3],[mx(x+.5),bottom],[mx(x),bottom+.5],[mx(x-.6),bottom-.1],[mx(x-.7),2]],i?p.shadow:p.base),line([[mx(x),-1.8],[mx(x+.1),3],[mx(x),bottom-.7]],p.light,.35));
    }
    for(let i=0;i<4;i++) {
      const x=-2.8+i*1.6,y=-3.8+Math.abs(x)*.13;
      front.push(line([[mx(x),y-.4],[mx(x+.2),y+.9],[mx(x-.25),y+1.6]] as [number,number][],p.shadow,.9),line([[mx(x-.25),y-.4],[mx(x-.1),y+.7]],p.light,.3));
    }
  } else {
    front.push(fill(cap,p.base));
    if(style==='mohawk') front.push(line([[0,-6.8],[.7,-4.8],[.6,-2.8]].map(([x,y]):Point=>[mx(x),y]),p.light,.55));
    else front.push(line([[-3.7,-1.4],[-2.7,-3.5],[-.6,-4.4],[1.6,-3.7]].map(([x,y]):Point=>[mx(x),y]),p.light,.55));
    if(style==='sidepart') front.push(line([[.6,-4.7],[.6,-3.3],[2,-1.9]].map(([x,y]):Point=>[mx(x),y]),p.shadow,.75));
    if(style==='undercut') front.push(line([[-3.7,-2.5],[-3.4,.1]].map(([x,y]):Point=>[mx(x),y]),p.shadow,1.2));
    if(style==='quiff') front.push(line([[-2.8,-4.4],[-1,-5.9],[1,-5.8],[2,-4.8]].map(([x,y]):Point=>[mx(x),y]),p.light,.6));
  }
  const bun=(x:number,y:number,r:number)=> {
    front.push(fill(cluster(x,y,r,8).map(([px,py])=>[mx(px),py]),p.shadow),fill(cluster(x-.25,y-.2,r*.8,7).map(([px,py])=>[mx(px),py]),p.base),line([[mx(x-r*.55),y],[mx(x-r*.25),y-r*.5],[mx(x+r*.35),y-r*.45]],p.light,.6));
    front.push(line([[mx(x-r*.7),y+r*.7],[mx(x+r*.6),y+r*.72]],'#bca073',.6));
  };
  ...
  const braid=(x:number,start:number,count:number)=> {
    for(let i=0;i<count;i++) {const y=start+i*1.55;front.push(fill([[mx(x-1.05),y],[mx(x+.1),y-.4],[mx(x+1.15),y+.5],[mx(x+.1),y+1.9],[mx(x-1),y+1.15]],i%2?p.shadow:p.base),line([[mx(x-.6),y+.3],[mx(x+.35),y+1]],p.light,.4));}
    front.push(line([[mx(x-.75),start+count*1.55],[mx(x+.8),start+count*1.55]],'#c6ad76',.7));
  };
