...
interface Request { id: number; seed: number; wildernessOnly?: boolean; riftTerrain?: boolean; authored?: boolean; x: number; y: number; }
...
scope.onmessage = ({ data }) => {
  try {
    if (!world || world.seed !== data.seed || world.wildernessOnly !== !!data.wildernessOnly || world.riftTerrain !== !!data.riftTerrain || (world instanceof AuthoredWorld) !== !!data.authored) {
      world?.dispose();
      world = data.authored ? new AuthoredWorld(data.seed) : new World(data.seed, !!data.wildernessOnly, !!data.riftTerrain);
    }
