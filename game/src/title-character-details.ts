...
export function titleCharacterLoading(tab: 'gear' | 'attributes'): string {
  ...
  return `<div class="title-selection-heading" aria-hidden="true"><h3>${line}</h3><div class="title-character-actions"><span class="ui-button ui-button--icon title-skeleton-slot"></span></div></div>
    ...
    <div class="title-cta-row" aria-hidden="true"><span class="ui-button title-skeleton-slot"></span><div class="ui-button title-enter title-skeleton-slot">${line}</div></div>`;
}
export function titleCharacterDetails(record: CharacterSave, derived: DerivedCharacterStats, tab: 'gear' | 'attributes', canEditAppearance = false): string {
  ...
  return `<div class="title-selection-heading"><h3>${escapeUI(displayName({ name: record.name, character: sheet }))}</h3>${canEditAppearance ? `<div class="title-character-actions"><button class="ui-button ui-button--quiet ui-button--icon" data-action="appearance" aria-label="Edit appearance" data-tooltip="Edit appearance" data-tooltip-align="end" data-tooltip-placement="below">${uiIcon('palette')}</button></div>` : ''}</div>
    ...
    <div class="title-cta-row"><button class="ui-button" data-action="delete">${uiIcon('trash')}<span>Delete character</span></button><button class="ui-button ui-button--primary title-enter" data-action="continue"><span><kbd class="title-continue-pad">A</kbd>Enter World</span>${uiIcon('chevron')}</button></div>`;
}
