import { RIFT_RULES, riftBonus, createRiftKey, riftRewardItemCount } from './rift-content.ts';
...
export function riftRewardItems(entrance:DungeonEntrance,playerLevel:number,elapsed?:number):Item[]{
  const tag=entrance.rift!,random=lcgRandom(entrance.seed^0x793ba189),luck=1+riftBonus(tag,'fortune')/100;
  ...
  const keyRandom=lcgRandom(entrance.seed^0x51c9a2bd),current=tag.keyTier??0;
