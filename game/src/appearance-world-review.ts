...
import { isWowRaceId, type WowRaceId } from './wow-types.ts';
...
const selected=params.has('sample')?Math.max(0,Math.min(9,Number(params.get('sample'))||0)):null;
let seed=9072026;
...
const samples=Object.values(WOW_CLASSES).map((wowClass,index)=>{
  const raceParam=params.get('race');
  const sheet=createCharacterSheet(wowClass.id,raceParam&&isWowRaceId(raceParam)?raceParam as WowRaceId:'human');
  sheet.look={appearance:{skin:pick(SKIN_PALETTES.filter(p=>WOW_RACES[sheet.raceId].skinTones.includes(p.id))).id,hairColor:pick(HAIR_PALETTES).id,hair:pick(HAIR_STYLES).id,facialHair:pick(FACIAL_HAIR).id,accessory:pick(ACCESSORIES).id},
    armorTints:Object.fromEntries(ARMOR_PARTS.map(p=>[p.id,pick(ARMOR_TINTS).id])),showHelmet:index===9};
  return {sheet,wowClass,index};
});
...
<header><div><h1>Character looks · In the world</h1><p>Ten seeded class recipes · Actual player rig, world lighting and CRT · World seed 7319</p></div><a href="/character-editor.html">Character editor</a></header><section class="${selected===null?'':'single'}"></section><footer>Frozen staged scene using the game renderer. No simulation ticks, combat, saves or cloud access. Gallery crops omit HUD; individual views include the native HUD.</footer>`;
try {
...
for(const sample of samples.filter(s=>selected===null||s.index===selected)){
  ...
  article.innerHTML=`<h2><a href="?sample=${sample.index}">${sample.index+1}. ${escapeUI(sample.wowClass.name)}</a></h2><p>${escapeUI(appearance.skin)} skin · ${escapeUI(appearance.hairColor)} ${escapeUI(appearance.hair)} · ${escapeUI(appearance.facialHair)} · ${escapeUI(appearance.accessory)}${sample.sheet.look.showHelmet?' · Helmet shown':''}</p>`;
