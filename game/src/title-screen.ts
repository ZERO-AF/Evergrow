...
export class TitleScreen {
  ...
  constructor(mount: HTMLElement, actions: TitleActions) {
    ...
    this.element.addEventListener('keydown', event => {
      ...
      if (target.matches('[data-slot]') && event.key === 'ArrowRight' && getComputedStyle(this.element.querySelector('.title-slot-grid')!).flexDirection === 'column') {
        const entry = [...this.element.querySelectorAll<HTMLElement>('[data-detail-tab],[data-title-item],.title-create input,.title-forge-stage input')].find(el => el.getClientRects().length && el.tabIndex >= 0);
    ...
    this.element.addEventListener('click', event => {
      ...
      if (action === 'appearance') {
        ...
        else if (slot?.state === 'empty') this.openAppearanceEditor();
      }
  ...
  private renderSelection() {
    ...
    this.player = previewCharacter(previewRecord ?? null, this.drafts?.get(this.selected)); this.portraitDirty = true;
  ...
  private setCreating(creating: boolean) {
    ...
    if (target && this.canvas && this.canvas.parentElement !== target) target.append(this.canvas);
  }
