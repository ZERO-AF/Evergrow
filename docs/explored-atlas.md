...
The full map now zooms out to **0.0003**, framing whole continents. Below 0.02 the chart draws the authored zone silhouettes and coastlines under a chunk-level exploration fog — unexplored zones stay dimmed but named, like the WoW world map — instead of sampling detail tiles. A scale rail gives the overview a readable distance reference. Native biome labels are placed only on revealed, predominantly matching terrain; they avoid settlement markers and reserve space from minor POIs. Labels are a cartographic aid, not discovery or simulation state.
