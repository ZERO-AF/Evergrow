...
Local September 19 Area journal: shared validation now retains uncapped accepted, known and completed Journey collections and allows a pin on an unaccepted activity. The obsolete dismissal suppression field is discarded from the parsed save copy; progress is preserved. The existing character size limit remains. A future requested publication must ship matching client and Worker validation; this local pass does not publish or change D1.

Local atlas rework · September 13, 2026: tree version 3 replaces the preceding atlas's connections. On a parsed copy, shared save decoding validates version-2 allocations against their published graph, including purchased ranks through 20, Techniques, exclusive Doctrines, bindings, cooldowns and the point ledger. Valid old builds receive all tree and purchased-rank points back and reopen the atlas with gameplay paused so skills can be unlocked and assigned again. Character, equipment, attributes, resources and world progress are preserved; the original stored bytes remain unchanged until a successful save. The preceding unversioned-tree refund remains supported. Publish the matching client and Worker together when requested; no D1 migration is needed. This local change has not been published.
...
The desktop hall uses a stable, viewport-bounded row during sign-in, loading and selection; long details scroll inside the panel. Loading placeholders share the responsive roster and character-detail sections. Available details and their portrait remain visible while refreshing, with detail actions disabled until the read finishes. Unloaded characters reserve the portrait space without showing a starter character; changing save source clears the previous source’s preview. The footer announces loading without moving the detail sections. Initial focus follows the selected character or active save source. Signed-out visitors use the in-panel sign-in action without a separate save-warning banner. Clicking the current source, character or home page keeps the current view without reloading. Switching selections, explicit retries, returning to the hall and background cloud revision changes still refresh saves.
