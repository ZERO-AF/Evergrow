...
Local September 19, 2026: the [WoW transformation](wow-transformation.md) lands the WotLK pass. Ten classes with their resource models (mana, rage, energy, runic power plus combo points, runes and soul shards) and ten races with passives and an auto-known racial active on R. Tab-targeting, auto-attack, the global cooldown, cast/channel bars, DoTs/HoTs, enemy crowd control (root, fear, incapacitate, polymorph, silence), stealth, taunt, pulls and pets/totems (15 ally templates, cap 6). The atlas gains ten class-gated sanctum clusters holding the 203 authored class skills and 30 class passives; the 30 weapon skills and 7 auras stay class-free. The title screen is a WoW-style race/class creation with a live starter-gear portrait; the HUD gains a target frame, player unit frame, cast bar, GCD sweeps, resource-colored orb and rune/shard/combo pips. Gear uses WoW naming pools and sixteen named legendaries; save v5 migrates v4 sheets with an inferred class and human race. Not yet published.
...
Local September 15 movement pass: ordinary player walking now gently steers around nearby obstacle edges when collision leaves little forward progress. `player-movement.ts` searches up to 18 world units to either side for a reachable route with 13 units of forward clearance, then chooses the smallest clear turn. Following player feedback, automatically steered steps use 60% of the requested walking speed (a 40% slowdown), with sharper sideways corrections slower still. The sideways component is capped at 65% of that reduced budget. Unassisted movement immediately regains its normal speed when the requested direction is clear; there is no lingering penalty. Short steps and additional path samples use the active world's collision rules, including dungeon room/corridor geometry. Useful existing wall slides remain intact. Assistance requires held movement and does not affect dodges, skill dashes, enemies, knockback or loot. No geometry/save changes or reset. Collision and simulation regressions cover trees, wall ends, doors, blocked gaps, release and dodge behavior; feel tuning awaits player testing.

Failed edge searches now use one per-player cache for at most 125 ms of simulation time. Ordinary collision checks still run every tick; successful slides are not cached. Position changes beyond 0.25 world unit or heading changes beyond two degrees bypass the failure, as do radius, world or clock changes. Tolerances stay anchored to the original failure so gradual movement cannot extend the cached region. Release/dodge/dash, relocation, reset/load and successful container destruction clear it. The cache is transient and never saved. A warmed desktop Node comparison (seven alternating batches, 6,000 calls per batch, advancing the 120 Hz simulation clock) reduced repeated generated-dungeon wall movement cost from about 0.32 to 0.026 ms per two ticks (about 92%); outdoor-wall cost fell about 90%. These are isolated CPU averages, not gameplay frame timings or Thor measurements; periodic retry ticks still perform a full search.

Movement PR review fixes: small controller variation now retains the failed-search cache (the two-second ±0.001 stick-input reproduction fell from 4,560 to 528 movement queries). Separate transient `locomotionVX/VY` values carry actual resolved travel into the shared pose, so foot direction and stride strength follow sliding instead of the input-smoothed velocity. Blocked/stopped, reset/restored and relocated poses settle correctly; dash/dodge presentation uses actual travel. Existing forest/water studies stage these same presentation values explicitly. The review fixes passed 85 focused movement, combat, dungeon, container, rig and appearance tests plus application/headless type checking and the production build. No save reset; visual/device acceptance remains with the player.
...
| --- | --- | --- |
| Characters and saves | Compact eight-slot hall, WoW race/class creation with live starter-gear portrait, level/power summaries and Continue/Create; local worker saves, portable character/chart import/export; Site-only Cloud / Local tabs with authenticated D1/R2 publication and a durable recovery outbox | Cloud deployment succeeded; real sign-in/cross-browser acceptance remains a player check; Android remains local-only; older sheets migrate to save v5 (v4 gains an inferred class and human race) |
| Combat | Deterministic 120 Hz simulation; tab-targeting, auto-attack, 1.5 s GCD (1.0 s rogue/cat), cast/channel spells, weapon basics, five assignable slots plus the R racial, dodge, dual potion; 250 active skills (30 weapon, 7 auras, 203 class, 10 racial); melee/bow attack speed and independent staff/wand/magic cast speed | Player tests feel and balance; no automatic combos or default assigned spell |
...
| Enemies | Eighteen archetypes (nine base, six regional, three dungeon bosses), three ranks, patrol/LOS, flank/pounce/ranged/area patterns, home return; hound patrol arrival; goblin rush/surround commands and leader-death morale; root/fear/incapacitate/polymorph/silence crowd control and taunt overrides | Hollow Warden remains dungeon-only; deeper elite modifiers remain future work |
| Enemy deaths | Four articulated death recipes for all eighteen enemy kinds (72 total), one random choice per kill, solid body parts and separate humanoid/hound/wisp rigs; shared `/deaths.html` review | Authored motion; neutral silhouette handoff, no physics ragdolls or terrain-aware corpse collisions; player visual acceptance pending |
...
| Skill atlas | Local 2,064-node, 2,578-edge open network; 144 passive neighborhoods, 30 skills, 90 Techniques, eight Doctrine families, four keystones and twenty ranks; ten class-gated sanctum clusters carry the class kits; stronger passive rewards and validated previous-tree refunds | Published v0.3.17 remains the 875-node atlas; open network is local and awaits further player balance feedback |
...
| World | Nine blended biomes, 29 prop families, seeded roads/rivers/lakes, water-aware settlements, streamed terrain and climate-specific environmental life | Finite coordinate/cache/save bounds; no weather or multi-site quest chains |
...
Every new character picks a race and class, then begins at level 1, 0 XP and 0 gold, with ten of each attribute, no unspent points, only the free tree origin allocated, five empty skill bindings and 64 empty bag cells. The same worn leather outfit accompanies the class's starter weapon and offhand from its starter table. Starter armor currently has no implicit/affix stat bonuses. Shared profiles determine weapon damage and cadence. See [character systems](character-systems.md) and [weapons and skills](weapons-and-skills.md).

Base life is 100; mana classes start at 100 mana regenerating at 1/second before bonuses, energy regenerates 10/second, and rage/runic power start empty, build through combat and decay out of combat. Q restores 42% maximum life and 40% maximum mana together, with two charges and a charge recovered every eight kills. Melee/bow basics cost no mana; staff basics cost four and wand basics two before reductions. First-row skills have no cooldown but still pay mana and obey action recovery; second-row skills cost more and have cooldowns. Attack speed, cast speed, mana-cost reduction and cooldown reduction remain distinct stats.
