...
Every enchanter offers Respec → Skills. A complete reset refunds every purchased tree node except the free origin and every additional purchased skill rank. Cost grows quadratically with refunded points — `respecCost(points) = min(50g, 80c·points²)` — so a light touch-up is cheap and a full level-80 rebuild approaches the 50-gold cap: 10 points cost ~0.8g, 40 cost ~12.8g, 79+ cost 50g. Unspent points are not charged. Attributes and equipment stay unchanged. The screen previews refunded points and exact cost before the explicit reset button. Reset clears skill assignments, selected ranks, specializations and Overload. Resources clamp to the new maxima without healing. Gold, refunded points and the new build save atomically; stale quotes and failed writes change nothing.

A separate Respec → Attributes choice offers one free refund per character for the September 11 offensive balance change. All assigned attribute points return to the unspent pool, and the four base attributes return to ten. Skills, equipment and gold remain unchanged. The saved `attributeResetUsed: true` flag prevents repeat claims; builds with no assigned points do not consume the offer. Quotes include allocations and unspent points; the same save-before-commit and resource-clamping rules apply.

## Expedition route

A physical chart table in settlements opens Expeditions. The table can be inspected before level 20, but starting requires level 20. Opening and starting share the same 75-unit reach and unobstructed approach check. Interaction traces to the nearest outside edge, so the solid tabletop does not block players approaching from behind. A real obstruction, distance failure or missing table has its own message; only an under-level character sees the level requirement. An expedition is a ten-stage character-owned route. Stages offer one or two seeded dungeon choices, with themes, enemy level, boss and modifiers visible before entry. Choosing a fork commits that branch. Entered floors retain their saved entrance, modifier, seed and level when the available modifier pool changes; resuming uses this snapshot. Choices and rewards cannot be rerolled by reopening, traveling or reloading.
