...
Stock refreshes at character levels **4, 7, 10, …**, globally for that character. The restock epoch is `floor((level - 1) / 3)`. Returning, reopening, waiting, reloading or unloading a town never refreshes stock. Purchased cells remain sold until the next epoch. Buyback survives restocking. Show only a small functional restock hint when needed. Paid refresh replaces the entire current merchant stock. The first costs `5 × (30 + 3 × (stock level − 1))` gold and each further refresh at that merchant doubles the fee. The fee/count resets at the next free three-level restock; it persists through reopening, travel and reload. At stock level 10: **285 → 570 → 1,140 → 2,280 gold**. Rarity weights, premium slots and regional stock levels stay unchanged.
...
## Spatial shops and workbench · local 2026-09-13
...
Merchant trading supports dragging a carried item onto the merchant pane to sell just that item immediately, including from Shop and Buyback. Stock and buyback pieces can be dragged into the bag to buy, or purchased with a double-click; single-click inspection and the footer action remain available. Purchased pieces use the normal safe inventory packing. During the drag, the destination highlights and shows the exact price or blocking reason. Equipment and enhancement views do not expose sale gestures. Canceling a drag changes nothing. The original quote is captured at drag start and validated by the existing durable trade on release; locked gear, stale quotes, insufficient gold, full packs and failed saves retain ownership and gold.
