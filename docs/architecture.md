...
`skill-tree.ts` generates 2,064 immutable nodes with 2,578 curved connections and world bounds. Six territories establish regional direction; short connectors, central notables and inner hybrid bridges provide alternate paths. The graph carries 144 passive neighborhoods, 30 weapon-skill groups, 90 Techniques, eight Doctrine families, four keystones and twenty ranks, plus a Class Sanctum ring: ten class-gated clusters whose nodes carry `classId` and reject foreign allocation. `skill-progression.ts` resolves purchased/active ranks, ninety Techniques and skill-specific leaf passives, mastery limits and Overload; commands validate their mutations and save v5 counts their point costs. The graph owns deterministic placement and stat recipes, not presentation. `skill-tree-routes.ts` computes the shortest additional-point route from the allocated build without mutating it. `skill-tree-art.ts` renders the culled curves, cluster labels, states, and route preview at native resolution; `skill-tree-glyphs.ts` shares code-defined stat/skill engravings between Canvas nodes and DOM details. Keep view transforms, zoom-dependent detail, and hover state out of allocation rules.

`skill-content.ts` owns the shared `SkillDefinition` schema — class/race gates, resource, rune/shard/combo costs, cast/channel, target mode, GCD and requirement flags — and merges the weapon skills, auras and `WOW_SKILLS` into `SKILL_DEFINITIONS` with weapon requirements, compatible-hand selection and shared icons. `skill-execution-content.ts` supplies deeply frozen profiles for sweeps, dashes, radial/cone attacks, guards, backstabs, projectiles, ground attacks and chains plus the WoW kinds (strike, dot, heal, hot, buff, cc, interrupt, pull, taunt, summon, channel, form, stealth, comboStrike, runeStrike, cleanse). Ranges, arcs, speeds, durations, status potency and numeric UI labels share those profiles. `skill-combat.ts` dispatches exhaustively by execution kind through a narrow context; adding a content variant no longer needs a new skill-name branch. Skill cooldowns are keyed by skill identity, so moving slots cannot reset them. `Simulation` preserves the fixed order and advances player cooldowns/lunges/guard, the GCD, casts/channels, resources, buffs and allies. `combat-status.ts` applies and ticks dots, slows, stuns and enemy crowd control; `ground-effects.ts` snapshots and advances delayed attacks. `combat-damage.ts` commits contacts/deaths and calls `combat-rewards.ts` for source-level XP, drops and flask recharge. Loot collection remains in Simulation. Ground loot art and HUD skill art remain presentation-only.

**WoW classes and races:** `wow-types.ts` owns the class/race identities, resource kinds, DoT/CC/buff specs, ally kinds and the tab-target/GCD constants. `wow-classes.ts` owns the ten class definitions — resource model, GCD, armor style, starter gear and HUD class colors. `wow-races.ts` owns the ten races — class availability, racial active and passive modifiers. `wow-skills-<class>.ts` (ten files) author the class kits and `wow-skills-racial.ts` the ten racial actives; `wow-skills.ts` aggregates them into `WOW_SKILLS`, which `skill-content.ts` merges into `SKILL_DEFINITIONS`. `wow-allies.ts` owns the fifteen pet/totem templates. `skill-tree-content.ts` arranges each kit into its class-gated sanctum cluster; allocation, routing and `resolveSkill` reject foreign classes/races. `item-naming.ts` owns the WoW gear name pools and the legendary table. See [the WoW transformation](wow-transformation.md).
...
`TitleScreen` presents eight slot records and the WoW-style race/class creation (class availability, starter-gear portrait, name and seed) and invokes application callbacks. `CharacterSession` owns the active slot, compatibility and writer token; `CharacterRepository` atomically stores validated records and retains the prior valid checkpoint. `character-save.ts` validates the entire sheet and world progress — including class/race identity, runes, shards, combo points, buffs and allies — before `Simulation.restoreCheckpoint` rebuilds projections. Character actions and application lifecycle boundaries trigger saves; the frame coordinator also writes every ten seconds. `character-summary.ts` derives display-only power, while `character-portrait.ts` shares the real equipped rig between the hall and inventory. See [save boundaries](character-saves.md).
